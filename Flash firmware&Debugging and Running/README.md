# K230 Vision Module — Quick Start Guide

## About `recognition.py`

`recognition.py` is a real-time object detection demo based on **YOLOv8n**, running on the K230 vision module. It uses the onboard camera to capture live video, performs AI inference via the K230's built-in KPU (Knowledge Processing Unit), and displays labeled bounding boxes on screen.

**How it works:**

- **Dual-channel camera output** — Channel 0 feeds the display at 640×480, while Channel 1 feeds the AI pipeline at 320×240 for efficient inference.
- **AI preprocessing** — `ai2d` handles letterbox padding and resize to match the model's 224×224 input resolution.
- **YOLOv8n inference** — The KPU runs a pre-compiled `.kmodel` (yolov8n_224.kmodel) to detect objects across 80 COCO categories.
- **Post-processing** — Non-Maximum Suppression (NMS) removes duplicate detections, then results are filtered for pet classes (bird, cat, dog — class IDs 14, 15, 16).
- **Visualization** — Bounding boxes and confidence scores are drawn on the OSD overlay and output to the display.
- **Recording (optional)** — Commented-out MP4 recording logic can automatically capture 20-second video clips when a pet is detected.

> **Prerequisites:** To run this code, you must first complete ALL of the following setup steps detailed below:
>
> 1. **[K230 Firmware Download and Flashing](#k230-firmware-download-and-flashing)** — Flash the CanMV K230 firmware onto a TF card.
> 2. **[Installing the Programming Environment](#installing-the-programming-environment)** — Install CanMV IDE K230 V4.0.0+ on your PC.
> 3. **[Debugging and Running Examples](#debugging-and-running-examples)** — Connect the K230 to the IDE, select the COM port, and learn how to run scripts.
>
> Once the firmware is flashed, the IDE is installed, and the device is connected, open `recognition.py` in CanMV IDE K230 and click **Run**.
